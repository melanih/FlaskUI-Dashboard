...
def index():
    ...
    top_category = pd.crosstab(
        index=df2['Category'],
        columns='Jumlah'
    ).sort_values(by='Jumlah', ascending=False).head()
    top_category = top_category.reset_index()
    # Dictionary stats digunakan untuk menyimpan beberapa data yang digunakan untuk menampilkan nilai di value box dan tabel
    stats = {
        'most_categories': top_category['Category'][0],
        'total': top_category['Jumlah'][0],
        'rev_table': df2.groupby(['Category', 'App']).mean().sort_values(by='Reviews', ascending=False).head(10).reset_index().drop(['Size', 'Installs', 'Price'], axis=1).to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])
    }
    ...
    playstore['Content Rating'] = playstore['Content Rating'].astype(
        'category')
